# Reference: https://github.com/yuxumin/PoinTr/blob/master/models/Transformer_utils.py
import torch
...
def knn_point_dist(nsample, xyz, new_xyz):
    """
    Input:
        nsample: max sample number in local region
        xyz: all points, [B, N, C]
        new_xyz: query points, [B, S, C]
    Return:
        dist: grouped points distance [B, S, nsample]
        group_idx: grouped points index, [B, S, nsample]
    """
    sqrdists = square_distance(new_xyz, xyz)
    dist, group_idx = torch.topk(sqrdists, nsample, dim = -1, largest=False)
