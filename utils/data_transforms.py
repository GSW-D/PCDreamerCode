...
class UpSamplePoints(object):
    ...
    def __call__(self, ptcloud):
        ...
        if need < 0:
            return ptcloud[np.random.choice(curr, self.n_points)]
...
class UpSamplePointsObjaverse(object):
    def __init__(self, parameters):
        self.n_points = parameters['n_points']
    ...
    def __call__(self, ptcloud):
        ...
        if need < 0:
            return ptcloud
            # return self.farthest_point_sampling(ptcloud, num_points=self.n_points)
    ...
    def farthest_point_sampling(self, points, num_points):
        """
        Farthest Point Sampling algorithm to select a subset of points.

        Parameters:
        - points: Input point cloud as a numpy array (N x 3).
        - num_points: Number of points to be selected.

        Returns:
        - sampled_points: Selected subset of points.
        """

        # Convert the input points to a numpy array
        points = np.array(points)

        # Initialize arrays to store selected indices and distances
        selected_indices = np.zeros(num_points, dtype=int)
        distances = np.ones(len(points)) * np.inf

        # Randomly select the first point
        selected_indices[0] = np.random.randint(len(points))
        distances = np.minimum(distances, np.linalg.norm(points - points[selected_indices[0]], axis=1))
